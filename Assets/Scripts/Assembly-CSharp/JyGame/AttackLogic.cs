...
namespace JyGame
{
	public class AttackLogic
	{
		...
		public static AttackResult GetAttackResult(SkillBox skill, BattleSprite sourceSprite, BattleSprite targetSprite, BattleField bf)
		{
			...
			double num9 = (double)skill.Power * (2.0 + (double)num5 / 200.0) * 2.5 * (4.0 + (double)role.AttributesFinal["bili"] / 120.0) * (1.0 + num6);
			double num10 = (double)skill.Power * (2.0 + (double)num5 / 200.0) * 2.5 * (4.0 + (double)role.AttributesFinal["bili"] / 120.0) * (1.0 + num6) * (double)(1f + equippedInternalSkill.Attack);
			double num11 = (double)role.AttributesFinal["fuyuan"] / 50.0 / 20.0 * (double)(1f + equippedInternalSkill.Critical) * (1.0 + num6);
			num11 += (double)role.CriticalProbabilityAdd();
			double num12 = 150.0 + (10.0 + (double)role2.AttributesFinal["dingli"] / 40.0 + (double)role2.AttributesFinal["gengu"] / 70.0) * 8.0 * (double)(1f + equippedInternalSkill2.Defence);
			...
			if (role.HasTalent("夫妻同心"))
			{
				foreach (BattleSprite sprite in field.Sprites)
				{
					if (sprite.Role != role && sprite.Role.Female != role.Female && sprite.Role.HasTalent("夫妻同心") && sprite.Team == role.Sprite.Team)
					{
						...
						num9 *= Tools.GetRandom(1.2, 2.0);
						num10 *= Tools.GetRandom(1.2, 2.0);
						break;
					}
				}
			}
			...
			if (role.HasTalent("阴谋家"))
			{
				...
				num9 *= 1.0 + 0.5 * (1.0 - num15);
				num10 *= 1.0 + 0.5 * (1.0 - num15);
			}
			...
			if (role.HasTalent("怒不可遏"))
			{
				...
				num9 *= 1.0 + (double)balls * 0.1;
				num10 *= 1.0 + (double)balls * 0.1;
			}
			...
			if (role.HasTalent("精打细算") && Tools.ProbabilityTest(0.25))
			{
				...
				num11 *= Tools.GetRandom(1.0, 2.0);
				num9 *= Tools.GetRandom(1.0, 1.5);
				num10 *= Tools.GetRandom(1.0, 1.5);
			}
			if (role.HasTalent("精明"))
			{
				...
				num11 *= Tools.GetRandom(1.0, 1.5);
				num9 *= Tools.GetRandom(1.0, 1.3);
				num10 *= Tools.GetRandom(1.0, 1.3);
			}
			...
			if (role.HasTalent("大小姐") || role.HasTalent("自我主义"))
			{
				...
				num9 *= (double)(1f + num18 * (float)num19);
				num10 *= (double)(1f + num18 * (float)num19);
			...
			{
				num11 -= 0.3 * (double)(role2.Level / 30);
			...
			{
				num9 *= 1.0 + 0.2 * ((double)role.level / 30.0);
				num10 *= 1.0 + 0.2 * ((double)role.level / 30.0);
			}
			...
			attackFormula.attackLow = num9;
			attackFormula.attackUp = num10;
			attackFormula.criticalHit = num11;
			attackFormula.defence = num12;
			LuaManager.Call("AttackLogic_extendTalents3", sourceSprite, targetSprite, skill, bf, attackResult, attackFormula);
			num9 = attackFormula.attackLow;
			num10 = attackFormula.attackUp;
			num11 = attackFormula.criticalHit;
			num12 = attackFormula.defence;
			...
			foreach (Trigger allTrigger in role.GetAllTriggers())
			{
				...
				{
				case "attack":
					num9 += double.Parse(allTrigger.Argvs[0]) / 2.0;
					num10 += double.Parse(allTrigger.Argvs[0]);
					break;
			...
			foreach (Trigger allTrigger2 in role2.GetAllTriggers())
			{
				...
				{
				case "defence":
					num12 += double.Parse(allTrigger2.Argvs[0]);
					num11 -= double.Parse(allTrigger2.Argvs[1]) / 100.0;
					if (num11 < 0.0)
					{
						num11 = 0.0;
					}
			...
			if (buff11 != null)
			{
				num9 *= 1.0 + (double)buff11.Level / 10.0;
				num10 *= 1.0 + (double)buff11.Level / 10.0;
			}
			if (buff12 != null)
			{
				num9 *= 1.0 - (double)buff12.Level / 10.0;
				num10 *= 1.0 - (double)buff12.Level / 10.0;
			}
			BuffInstance buff13 = targetSprite.GetBuff("伤害加深");
			if (buff13 != null)
			{
				num9 *= Math.Pow(1.15, buff13.Level);
				num10 *= Math.Pow(1.15, buff13.Level);
			}
			...
			if (buff14 != null)
			{
				num12 += (double)((buff14.Level + 1) * 20);
			}
			double num29 = defenceDescAttack(num12);
			bool flag2 = Tools.ProbabilityTest(num11);
			...
			attackResult.Hp = (int)(Tools.GetRandom(num9, num10) * ((!flag2) ? 1.0 : num24) * (1.0 - num29));
