...
public class CameraWrap
{
	...
	private static int get_opaqueSortMode(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, camera.opaqueSortMode);
		return 1;
	}
	...
	private static int get_cameraType(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, camera.cameraType);
		return 1;
	}
	...
	private static int get_stereoMirrorMode(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, camera.stereoMirrorMode);
		return 1;
	}
	...
	private static int get_targetDisplay(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, camera.targetDisplay);
		return 1;
	}
	...
	private static int get_commandBufferCount(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, camera.commandBufferCount);
		return 1;
	}
	...
	private static int set_onPreCull(IntPtr L)
	{
		...
		else
		{
			...
			Camera.onPreCull = delegate(Camera param0)
			{
	...
	private static int set_onPreRender(IntPtr L)
	{
		...
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			Camera.onPreRender = delegate(Camera param0)
			{
				int oldTop = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
				func.PCall(oldTop, 1);
	...
	private static int set_onPostRender(IntPtr L)
	{
		...
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			Camera.onPostRender = delegate(Camera param0)
			{
				int oldTop = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
				func.PCall(oldTop, 1);
	...
	private static int set_opaqueSortMode(IntPtr L)
	{
		...
		camera.opaqueSortMode = (OpaqueSortMode)(int)LuaScriptMgr.GetNetObject(L, 3, typeof(OpaqueSortMode));
		return 0;
	}
	...
	private static int set_cameraType(IntPtr L)
	{
		...
		camera.cameraType = (CameraType)(int)LuaScriptMgr.GetNetObject(L, 3, typeof(CameraType));
		return 0;
	}
	...
	private static int set_stereoMirrorMode(IntPtr L)
	{
		...
		camera.stereoMirrorMode = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
	...
	private static int set_targetDisplay(IntPtr L)
	{
		...
		camera.targetDisplay = (int)LuaScriptMgr.GetNumber(L, 3);
		return 0;
	}
	...
	private static int AddCommandBuffer(IntPtr L)
	{
		LuaScriptMgr.CheckArgsCount(L, 3);
		Camera camera = (Camera)LuaScriptMgr.GetUnityObjectSelf(L, 1, "Camera");
		CameraEvent evt = (CameraEvent)(int)LuaScriptMgr.GetNetObject(L, 2, typeof(CameraEvent));
